...
BUILD_DIR="out/FingerBrowserKernelWinX64"
...
GN_ARGS='target_os="win" target_cpu="x64" is_debug=false is_component_build=false symbol_level=0 enable_nacl=false proprietary_codecs=false'
...
case "$(uname -s)" in
  MINGW*|MSYS*|CYGWIN*) ;;
  *)
    echo "This build script is pinned to a dedicated Windows x64 builder" >&2
    exit 1
    ;;
esac

if [[ "${PROCESSOR_ARCHITECTURE:-}" != "AMD64" && "$(uname -m)" != "x86_64" ]]; then
  echo "This build script is pinned to a dedicated Windows x64 builder" >&2
  exit 1
fi

# External builders compile with a locally installed Visual Studio instead of
# Google's internal packaged toolchain. That path requires DEPOT_TOOLS_WIN_TOOLCHAIN=0.
if [[ "${DEPOT_TOOLS_WIN_TOOLCHAIN:-}" != "0" ]]; then
  echo "Set DEPOT_TOOLS_WIN_TOOLCHAIN=0 to build with a locally installed Visual Studio" >&2
  exit 1
fi
...
echo "Built Chromium runtime at $SRC_DIR/$BUILD_DIR/chrome.exe"
