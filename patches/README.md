...
8. ~~**`fp-passkey` 尚未从 151 移植**~~ —— **已移植**（124 树提交 `a8089735c3`），
   ...
   **症状的描述要小心**：无认证器时 WebAuthn 调用会异常失败，但**具体形态随条件
   变化**，不要写死成「25 秒挂起」。实测：

   | 环境 | 结果 |
   |---|---|
   | 本地探针（124 / 151，localhost，headless 或有窗口） | HANG 15–25s |
   | 客户端实测（151，真实环境，figma.com） | REJECT 1ms `NotAllowedError` |

   两者都是坏的，但用户看到的不同：一个是页面卡住，一个是站点报「设备不支持」。
   成因未查清 —— 已排除 attachment、`userVerification`（三种取值都挂）、
   headless vs 有窗口、用户手势（四项各自单独变更后仍挂）。未测的差异是真实
   HTTPS 源 vs `localhost`，以及对照组是否带着其余策略字段。

   **写死一种症状的代价**：下一个人按「25 秒挂起」去复现，在真实站点上会得到
   「没问题」，从而认为缺陷不存在。
