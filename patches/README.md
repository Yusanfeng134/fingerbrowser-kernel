...
7. ~~**`gpu_adapter.cc` 的注释引用了 151 才有的 `info` 属性**~~ —— **已修**
   ...
   与第 3 条对照着看很有意思，两者形状不同：**3 是源头就坏（151 的断言本身是错的，
   两边都得修）；7 是源头是对的**（151 的注释准确描述了 151 的结构），搬到 124 后
   描述的对象变了而注释没跟着变。**抄代码时既要防「抄到坏的」，也要防「抄到对的、
   但它对新宿主不再成立」。**

   顺带查清并写进注释的两条真事实：`requestAdapterInfo()` 的非 dev 分支**只暴露
   vendor 与 architecture**；由此**用开了 dev-features 的实例去验会掩盖 web 路径上
   的泄漏**。
