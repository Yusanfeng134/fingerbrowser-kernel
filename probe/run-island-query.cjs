...
// ── 它看不见什么，以及谁看得见 ──────────────────────────────────────────
//
// 这个脚本只读源码，不跑内核 —— 所以它**通过不代表你正在跑的内核里那行是对的**。
// 源码和已同步的二进制可以不一致（构建成功但同步失败就是，实际发生过）。
//
// 客户端的 `app:status.kernel.builtAt` 恰好相反：它取 yunbrowser.exe 与主资源包
// 里最新的那个时间戳，完全不知道源码里写了什么，只知道「我跑的这份是什么时候
// 构建的」。
//
//   本脚本        看得见：源码里那行对不对    看不见：跑的是不是这份源码
//   builtAt       看得见：跑的是哪次构建      看不见：那次构建里代码对不对
//
// **任何一侧单独都能给出「一切正常」的错觉。** 要断言「正在跑的内核里这行是
// 对的」，两个都得看 —— 或者用本目录里启动内核的那一类脚本端到端跑一次。
//
// （builtAt 取「最新」而不是「exe 的时间」，是因为纯资源改动 exe 根本不变，
// 只看 exe 会漏报 —— 与本目录 README 里记的那次 chrome.dll 判据同源。）
