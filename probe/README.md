...
| `run-hover-screendetailed.cjs` | 0041 两处第二读点 | 判据是**真机不可达的组合不出现** |
...
- 伪装值与宿主真值撞在一起：把 DPR 伪装成 1.0，而 headless 宿主的 DSF 恰好就是 1，
  「已伪装」这个断言恒真。**这条是探针自己的前提检查抓到的** —— 它拒绝给出一个
  没有意义的绿灯，改成 2.0 才有判别力
